...
class NewGiftSentNotification extends Notification {
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.gift_sent')
            ->adminMessage('panel.notifications.gift_sent')
            ->parse();
    }

    public function getBody($notifiable) {
        return  NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.gift_send_body', ['ID' => $this->gift->id,'sender_name' => $this->gift->sender->name,'receiver_name' => $this->gift->receiver->name])
            ->adminMessage('panel.notifications.gift_send_body', ['ID' => $this->gift->id,'sender_name' => $this->gift->sender->name,'receiver_name' => $this->gift->receiver->name])
            ->parse();
    }
}
