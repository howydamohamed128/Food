...
class ReplyContactUsNotification extends Notification
{
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.new_reply_contact_us_title', ['id' => $this->id])
            ->adminMessage('panel.notifications.new_reply_contact_us_title', ['id' => $this->id])
            ->parse();
    }

    public function getBody($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.new_reply_contact_us_body:reply', ['reply' => $this->contact->reply])
            ->adminMessage('panel.notifications.new_reply_contact_us_body:reply', ['reply' => $this->contact->reply])
            ->parse();
    }
}
