...
class OrderRatedNotification extends Notification {
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->adminMessage('panel.notifications.order_rated', ['id' => $this->order->id])
            ->parse();
    }

    public function getBody($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->adminMessage('panel.notifications.order_rated_body', ["order_number" => $this->order->order_number, 'customer_name' => $this->order->customer->name])
            ->parse();
    }
}
