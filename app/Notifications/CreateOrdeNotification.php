...
class CreateOrdeNotification extends Notification {
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.create_order', ['id' => $this->id])
            ->adminMessage('panel.notifications.create_order', ['id' => $this->id])
            ->parse();
    }

    public function getBody($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.create_order_body', ["order_number" => $this->order->order_number, 'customer_name' => $this->order->customer->name])
            ->adminMessage('panel.notifications.create_order_body', ["order_number" => $this->order->order_number, 'customer_name' => $this->order->customer->name])
            ->parse();
    }
}
