...
class ManualReservationCreatedNotification extends Notification {
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.manual_reservation_created')
            ->parse();
    }

    public function getBody($notifiable) {
        return  NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.manual_reservation_created_body',['id' => $this->order->id])
