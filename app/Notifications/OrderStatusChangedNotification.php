...
class OrderStatusChangedNotification extends Notification {
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ...
            ->adminMessage('panel.notifications.order_status_changed', ['id' => $this->order->id])
            ->parse();
    }

    public function getBody($notifiable) {
        return [
            'ar' => __('panel.notifications.order_status_changed_body', ['status' => $this->order->status->getLabel('ar'), 'ID' => $this->order->order_number],'ar'),
            'en' => __('panel.notifications.order_status_changed_body', ['status' => $this->order->status->getLabel('en'), 'ID' => $this->order->order_number],'en'),
        ];
