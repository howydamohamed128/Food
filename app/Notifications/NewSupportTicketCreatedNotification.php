...
class NewSupportTicketCreatedNotification extends Notification {
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->adminMessage('panel.notifications.new_support_ticket',)
            ->parse();
    }

    public function getBody($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->adminMessage('panel.notifications.new_support_ticket_body', ['customer_name' => $this->supportTicket->user->name])
            ->parse();
    }
}
